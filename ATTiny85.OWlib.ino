...
unsigned char rom[8] = {0x28, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00};       //0x28 - DS18B20 identificator
...
void setup() {
    ...
    digitalWrite(debugPin, debug);  
    pinMode(debugPin2, OUTPUT);
    digitalWrite(debugPin2, debug);  
...
void loop() {
    ...
    if (owReset) {
      owHandler();    // Handle the OW reset that was received
    }
}
...
void onewireInterrupt(void) {
  //digitalWrite(debugPin, HIGH);
    ...
    prevInt = micros();
    /* Ubral extra call of micros() from the original code */ 
    //volatile long now = micros();
    //volatile long lastMicros = now - prevInt;
    //prevInt = now;
